...
private:
	...
	// Newly added and allowable string input is stored in this vector
	std::vector<std::string> candVector;

	// Palindrome vectors (solutions) are stored in this vector
...
public:
	...
	std::vector<std::string> getCandVector() const;

	std::string vectorToString(const std::vector<std::string> & stringVector);
